...
class CameraSelectionWindow:
    ...
    def __init__(self, parent, controller: CameraSelectionController,
        ...
        self.window.geometry("620x610"); self.window.protocol("WM_DELETE_WINDOW", self.close)
        ...
        ttk.Combobox(self.network_form, textvariable=self.network_type,
                     values=("RTSP", "HTTP/MJPEG", "Personalizada"), state="readonly", width=18).grid(row=1, column=1, sticky="w", padx=5)
        ttk.Label(self.network_form, text="URL").grid(row=2, column=0, sticky="w")
        ttk.Entry(self.network_form, textvariable=self.network_url, width=42, show="•").grid(row=2, column=1, sticky="ew", padx=5)
        self.network_form.columnconfigure(1, weight=1)
        network_actions = ttk.Frame(self.network_form); network_actions.grid(row=3, column=0, columnspan=2, sticky="ew", pady=(8, 0))
    ...
    def test_network_form(self) -> None:
        ...
        try: available = self.controller.probe_network_source(
    ...
    def _network_source_type(self) -> CameraSourceType:
        return (CameraSourceType.NETWORK_RTSP if self.network_type.get() == "RTSP" else
                CameraSourceType.NETWORK_HTTP if self.network_type.get() == "HTTP/MJPEG" else
