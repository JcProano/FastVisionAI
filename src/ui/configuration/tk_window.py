...
class ConfigurationWindow:
 ...
 def _restore(self):self.editor.delete("1.0","end");self.editor.insert("1.0",json.dumps(self.loaded.as_mapping(),indent=2,sort_keys=True))
 def validate(self):
  try:
 ...
 def import_file(self):
  ...
  try:
   ...
   if messagebox.askyesno("Importar configuración",summary,parent=self.window):self.editor.delete("1.0","end");self.editor.insert("1.0",json.dumps(candidate,indent=2,sort_keys=True))
