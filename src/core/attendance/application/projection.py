...
def project_days(rows: tuple[AttendanceRecord, ...], policy: AttendancePolicy,
    ...
    grouped: dict[tuple[str, date], list[AttendanceRecord]] = {}
    for row in rows:
        key = row.person_id, row.timestamp.astimezone(zone).date()
    ...
    return tuple(
        sorted(
            projected,
            key=lambda item: (item.local_date, item.updated_at),
            reverse=True,
        )
    )
...
def _day(person_id: str, day: date, rows: tuple[AttendanceRecord, ...],
    ...
    worked = (
        max(0, int((check_out.timestamp - check_in.timestamp).total_seconds()))
    ...
    if outs and not check_in:
        status = AttendanceDayStatus.INCOMPLETE
    elif manual:
        status = AttendanceDayStatus.MANUAL_ADJUSTMENT
...
def today_summary(days: tuple[AttendanceDayRecord, ...], latest: tuple[AttendanceRecord, ...],
    ...
    return AttendanceTodaySummary(
        day,
        len(days),
        sum(item.check_out_utc is not None for item in days),
        sum(
            item.check_in_utc is not None and item.check_out_utc is None
            for item in days
        ),
        sum(item.late_seconds > 0 for item in days),
        latest[:5],
    )
...
def monthly_summary(days: tuple[AttendanceDayRecord, ...], person_id: str,
    ...
    return AttendanceMonthlyPersonSummary(
        person_id,
        year,
        month,
        len(selected),
        sum(item.late_seconds > 0 for item in selected),
        sum(item.worked_seconds for item in selected),
