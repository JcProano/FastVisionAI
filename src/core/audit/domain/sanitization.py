"""Strict sanitization policy for audit text and flat metadata."""
...
from .models import AuditValidationError

_SENSITIVE = re.compile(
    r"password|hash|salt|secret|token|api[_-]?key|credential|private[_-]?key",
    re.I,
)
    ...
    *,
    ...
    for raw_key, raw_value in value.items():
        ...
        if _SENSITIVE.search(key):
            ...
            continue
        if isinstance(
            raw_value, (dict, list, tuple, set, bytes, bytearray, Path)
        ):
