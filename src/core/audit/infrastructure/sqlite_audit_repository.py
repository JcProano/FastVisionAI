"""Append-only SQLite adapter for administrative audit records."""
...
class SQLiteAuditRepository:
    def __init__(
        self, database_path: Path, *, timeout: float = 5.0
    ) -> None:
        if timeout <= 0:
    ...
    def initialize(self) -> int:
        ...
        try:
            ...
            raise AuditRepositoryError(
                "audit database initialization failed"
            ) from exc
        finally:
            connection.close()
    ...
    def append(self, record: AuditRecord) -> AuditRecord:
        ...
        try:
            ...
            connection.rollback()
            raise AuditRepositoryError(
                "audit record could not be appended"
            ) from exc
        finally:
            connection.close()
    ...
    def query(
        ...
        filters = (
            ...
            ("action = ?", query.action.value if query.action else None),
            ("actor_user_id = ?", query.actor_user_id),
            ("actor_role = ?", query.actor_role),
            (
                "entity_type = ?",
                query.entity_type.value if query.entity_type else None,
            ),
        ...
        try:
            rows = connection.execute(
                ...
                + where
                + " ORDER BY timestamp_utc DESC,audit_id DESC LIMIT ? OFFSET ?",
                parameters,
            ).fetchall()
            return tuple(_record(row) for row in rows)
        except Exception as exc:
            raise AuditRepositoryError("audit query failed") from exc
        finally:
            connection.close()
    ...
    def summary(self) -> AuditSummaryDTO:
        ...
        try:
            row = connection.execute(
                "SELECT COUNT(*),COALESCE(SUM(success),0),MAX(timestamp_utc) "
                "FROM audit_records"
            ).fetchone()
            total = int(row[0])
            successes = int(row[1])
            latest = datetime.fromisoformat(row[2]) if row[2] else None
            return AuditSummaryDTO(
                total, successes, total - successes, latest
            )
        except Exception as exc:
            raise AuditRepositoryError("audit summary failed") from exc
        finally:
            connection.close()
...
def _record(row: sqlite3.Row) -> AuditRecord:
    return AuditRecord(
        ...
        AuditEntityType(str(row["entity_type"])),
        row["entity_id"],
        bool(row["success"]),
        str(row["message"]),
        str(row["source"]),
