"""Validated JSON configuration loader adapter."""
...
class JSONConfigurationLoader:
    ...
    def load(
        ...
        try:
            ...
            raise ConfigurationError(
                "La configuración no pudo cargarse."
            ) from exc
        validation = self.validator.validate(value, profile)
        if not validation.valid:
            raise ConfigurationError(
                "La configuración no superó la validación."
            )
    ...
    def from_candidate(
        ...
        source_name: str = "candidate.json",
    ) -> ConfigurationSnapshot:
        validation = self.validator.validate(candidate, profile)
        if not validation.valid:
            raise ConfigurationError(
                "La configuración candidata no es válida."
            )
