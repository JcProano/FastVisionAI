"""Transactional SQLite adapter for administrative users only."""
...
from .migrations import SecurityMigrationError, initialize_schema
...
class SQLiteUserRepository:
    """Independent repository using one explicitly closed connection per call."""
    ...
    def initialize(self) -> int:
        ...
        except SecurityMigrationError:
            raise
        except Exception as exc:
            raise UserRepositoryError(
                "users database could not be initialized"
            ) from exc
    ...
    def list_users(self) -> tuple[UserDTO, ...]:
        with self._read_connection() as connection:
            rows = connection.execute(
                "SELECT * FROM users ORDER BY username COLLATE NOCASE,user_id"
            ).fetchall()
    ...
    def update_user(self, request: UserUpdateRequest) -> UserDTO:
        with self._transaction() as connection:
            ...
            connection.execute(
                ...
                "WHERE user_id=?",
                (
                    request.display_name or current.display_name,
                    new_role.value,
                    _utc_now().isoformat(),
                    current.user_id,
                ),
        ...
        *,
        ...
        with self._transaction() as connection:
            ...
            connection.execute(
                ...
                password_algorithm=?,password_parameters=?,password_changed_at=?,
    ...
    def _insert(
        ...
        try:
            connection.execute(
                ...
                password_algorithm,password_parameters,role,status,failed_attempts,
    ...
    @staticmethod
    def _ensure_other_admin(
        connection: sqlite3.Connection, excluded_id: str
    ) -> None:
    ...
    def _row(row: sqlite3.Row) -> UserRecord:
        try:
            return UserRecord(
                ...
                _parse(row["locked_until"]),
                _parse(row["last_login_at"]),
                _parse(row["password_changed_at"]),
