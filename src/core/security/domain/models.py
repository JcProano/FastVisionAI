...
class UserRepositoryError(SecurityError):
    pass


class DuplicateUsernameError(UserRepositoryError):
    pass
...
class AuthorizationPermission(str, Enum):
    ...
    VIEW_DETECTION_HISTORY = "VIEW_DETECTION_HISTORY"
...
class PasswordPolicy:
    ...
    def validate(self, password: str) -> None:
        if (
            ...
            or not self.minimum_length <= len(password) <= self.maximum_length
        ):
...
class UserRecord:
    ...
    password_hash: bytes = field(repr=False)
    password_salt: bytes = field(repr=False)
    password_algorithm: str = field(repr=False)
    password_parameters: str = field(repr=False)
...
class UserUpdateRequest:
    ...
    def __post_init__(self) -> None:
        ...
        if self.display_name is not None:
            object.__setattr__(
                self, "display_name", safe_name(self.display_name)
            )
...
def canonical_uuid(value: str) -> str:
    try:
        ...
        raise SecurityValidationError("user_id is invalid") from exc
    if str(parsed) != value.strip().lower():
        raise SecurityValidationError("user_id is invalid")
    return str(parsed)


def normalize_username(value: str) -> str:
    cleaned = value.strip().casefold()
    if not re.fullmatch(r"[a-z0-9._-]{3,64}", cleaned):
        raise SecurityValidationError("username is invalid")
    return cleaned


def safe_name(value: str) -> str:
    cleaned = " ".join(value.split())
    if not cleaned or len(cleaned) > 120 or any(
        ord(character) < 32 for character in cleaned
    ):
        raise SecurityValidationError("display name is invalid")
    return cleaned
