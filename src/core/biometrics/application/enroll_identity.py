"""Use case for deterministic, transactional biometric enrollment."""
...
class EnrollIdentityUseCase:
    """Validates a complete batch before writing and verifies compensation."""

    def __init__(
        self,
        gallery: BiometricGalleryPort,
        identity_factory: BiometricIdentityFactoryPort,
        embedding_math: EmbeddingMathPort,
        ...
        *,
        ...
        self._embedding_math = embedding_math
    ...
    def execute(
        ...
        person_id: str,
        display_name: str,
        embeddings: Sequence[BiometricEmbeddingPort],
        metadata: Mapping[str, Any] | None = None,
    ) -> EnrollmentResult:
        started = self._monotonic()
        identity = self._identity_factory.create(
            person_id, display_name, _copy_metadata(metadata)
        )
        with self._lock:
            before = _gallery_signature(self.gallery)
            if any(
                item.person_id == person_id
                for item in self.gallery.list_identities()
            ):
            ...
            accepted: list[tuple[int, BiometricEmbeddingPort]] = []
            rejected: list[RejectedEnrollmentTemplate] = []
            similarities: list[float] = []
            provenance = _gallery_provenance(self.gallery)
            fingerprints = {
                item.fingerprint for item in self.gallery.templates()
            }
            for input_index, embedding in enumerate(embeddings):
                ...
                if provenance is not None:
                    causes.extend(
                        _provenance_causes(provenance, candidate_provenance)
                    )
                if (
                    _enum_value(embedding.alignment_quality) == "low_quality"
                    and not self.policy.allow_low_quality
                ):
                    causes.append(EnrollmentCause.LOW_QUALITY)
                fingerprint = self._embedding_math.fingerprint(embedding)
                ...
                if not causes:
                    pair_values = [
                        self._embedding_math.similarity(embedding, previous)
                    ...
                    if (
                        ...
                        and any(
                            value < self.policy.min_pairwise_similarity
                    ...
                    if (
                        ...
                        and any(
                            value > self.policy.max_pairwise_similarity
                            for value in pair_values
                        )
                    ):
            ...
            try:
                ...
                for input_index, embedding in accepted:
                    gallery_indices.append(
                        self.gallery.add_template(
                            person_id,
                            embedding,
                            source_reference=(
                                f"enrollment-input-{input_index}"
                            ),
                        )
                    )
            except Exception:
            ...
            accepted_contracts = tuple(
                AcceptedEnrollmentTemplate(
                    input_index,
                    embedding.face_index,
                    embedding.alignment_quality,
                    gallery_index,
                )
                for (input_index, embedding), gallery_index in zip(
                    accepted, gallery_indices
                )
            )
    ...
    def _result(
        self,
        identity: object,
        accepted: Sequence[tuple[int, BiometricEmbeddingPort]],
        rejected: Sequence[RejectedEnrollmentTemplate],
    ...
    def _result_contract(
        self,
        identity: object,
        accepted: tuple[AcceptedEnrollmentTemplate, ...],
        rejected: tuple[RejectedEnrollmentTemplate, ...],
        causes: tuple[EnrollmentCause, ...],
        status: EnrollmentStatus,
        received: int,
        similarities: Sequence[float],
        ...
        metrics = EnrollmentMetrics(
            ...
            low_quality_rejected=rejected_causes.count(
                EnrollmentCause.LOW_QUALITY
            ),
            exact_duplicates_rejected=rejected_causes.count(
                EnrollmentCause.EXACT_DUPLICATE
            ),
            incompatible_rejected=sum(
                cause in _INCOMPATIBLE for cause in rejected_causes
            ),
            diversity_rejected=sum(
                cause
                in {
                    EnrollmentCause.INSUFFICIENT_DIVERSITY,
                    EnrollmentCause.INSUFFICIENT_SIMILARITY,
                }
                for cause in rejected_causes
            ),
            max_limit_rejected=rejected_causes.count(
                EnrollmentCause.MAX_TEMPLATES_EXCEEDED
            ),
        ...
        return EnrollmentResult(
            identity, accepted, rejected, causes, status, metrics
        )


_INCOMPATIBLE = {
    EnrollmentCause.INCOMPATIBLE_DIMENSION,
    EnrollmentCause.INCOMPATIBLE_MODEL,
    EnrollmentCause.INCOMPATIBLE_VERSION,
    EnrollmentCause.INCOMPATIBLE_WEIGHTS,
}
...
def _gallery_signature(gallery: BiometricGalleryPort) -> tuple[object, ...]:
    identities = tuple(
        (
            item.person_id,
            item.display_name,
...
def _provenance_causes(
    expected: tuple[int, str, str, str],
    actual: tuple[int, str, str, str],
) -> list[EnrollmentCause]:
    causes = []
