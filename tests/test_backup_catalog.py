...
class BackupCatalogTests(unittest.TestCase):
 def test_resolves_allowlisted_sources(self):
  with TemporaryDirectory() as d:
   catalog=BackupSourceCatalog(Path(d),{"backup":{"include_configuration":False}});self.assertEqual(len(catalog.sources()),7);self.assertTrue(any(item.component_type.value=="AUDIT_DATABASE" for item in catalog.sources()));self.assertTrue(all(Path(d).resolve() in s.source_path.parents for s in catalog.sources()))
