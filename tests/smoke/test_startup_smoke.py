...
class StartupSmokeTests(unittest.TestCase):
 ...
 def test_main_orders_login_before_real_adapter(self):
  source=(Path(__file__).resolve().parents[2]/"src/ui/main.py").read_text();self.assertLess(source.index("LoginWindow(root, security).run()"),source.index("adapter = RealUIRuntimeAdapter"))
